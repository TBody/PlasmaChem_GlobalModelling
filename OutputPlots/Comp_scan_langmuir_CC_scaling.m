...
cplt = semilogy(Scan_values*xlabel_multiplier, Density(Find_in_SI2E('e'),:)*1.7);
...
ebar = plot(expt_Scan_values*xlabel_multiplier, expt_Density_electron * normalisation, '.');
...
cplt = plot(Scan_values*xlabel_multiplier,Te+1,'--','LineWidth',Computational_line_width,'DisplayName','Te');
...
ebar = plot(H2_supply, kTe, '.');
% ebar = plot(H2_supply, Te_eedf, 'x')
ebar.Color = MATLAB_colours(2,:);
ebar.MarkerSize = 20;
ebar.LineWidth = Experimental_line_width;

leg = legend({'Density \times 1.7 (comp.)','Density (expt.)','T_e + 1eV (comp.)','T_e (expt.)'},'Location','northwest');
