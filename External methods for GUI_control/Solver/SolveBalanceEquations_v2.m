function [FinalDensity,DensityError,FinalTe,TeError,FinalRate,Converged] = SolveBalanceEquations_v2(Controller,RelTol,MaxStep,ConvergencePrecision,PreviousDensity,PreviousTe)
	%%Use custom warnings.
	...
	% Always use initial conditions
	InitialConditions 	= [PreviousDensity;PreviousTe];
	% if nargin == 6
	% 	InitialConditions 	= [FinalDensity';FinalTe];
	% else
	...
	[~,R_ConcTe] 	 = ode15s(@BalanceEquations,[0 Inf],InitialConditions,options);
	%%Export everything as column vectors
	FinalDensity     = R_ConcTe(end,1:end-1)';
	DensityError     = (abs(dDensity)./Density);
	FinalTe          = R_ConcTe(end,end);
	TeError          = abs(dTe)./Te;
